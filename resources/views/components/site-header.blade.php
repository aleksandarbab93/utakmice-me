...
<header class="border-b border-white/[0.07]">
    ...
    <div class="lg:hidden h-14 flex items-center justify-between px-4">
        @if ($active === 'home')
            <a href="{{ route('home', $sport) }}" class="flex items-center gap-2">
                <x-logo />
            </a>
        ...
        @elseif ($active === 'scores')
            <span class="text-xl font-extrabold tracking-tight">Rezultati</span>
    ...
    @if ($active !== 'standings')
        <div class="lg:hidden px-4 pb-3 flex gap-1.5">
            <a href="{{ route($active === 'scores' ? 'scores' : 'home', 'fudbal') }}"
            ...
            <a href="{{ route($active === 'scores' ? 'scores' : 'home', 'kosarka') }}"
    ...
    <div class="hidden lg:flex h-16 items-center justify-between px-7">
        <div class="flex items-center gap-7">
            <a href="{{ route('home', $sport) }}" class="flex items-center gap-2.5">
                <x-logo size="md" />
            </a>
            <div class="flex gap-1 bg-surface border border-white/[0.08] rounded-[10px] p-1">
                <a href="{{ route($active === 'scores' ? 'scores' : ($active === 'standings' ? 'standings' : 'home'), 'fudbal') }}"
                ...
                <a href="{{ route($active === 'scores' ? 'scores' : ($active === 'standings' ? 'standings' : 'home'), 'kosarka') }}"
            ...
            <nav class="flex gap-5 text-[13.5px] text-text-2">
                <a href="{{ route('scores', $sport) }}" class="{{ $active === 'scores' ? 'text-text font-semibold' : '' }}">Rezultati</a>
                <a href="{{ route('standings', $sport) }}" class="{{ $active === 'standings' ? 'text-text font-semibold' : '' }}">Tabele</a>
                <a href="{{ route('home', $sport) }}">Vesti</a>
                <a href="#">Prenosi</a>
            </nav>
        </div>
        <div class="w-64 h-9 rounded-full bg-surface border border-white/[0.08] flex items-center gap-2 px-3.5 text-text-dim text-[13px]">
            ...
            <span>Pretraga klubova i vesti</span>
        </div>
    </div>
</header>
