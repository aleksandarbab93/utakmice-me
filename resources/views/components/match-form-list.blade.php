...
<div class="flex flex-col gap-2.5">
    ...
    <div class="bg-surface border border-white/[0.07] rounded-2xl overflow-hidden">
        @forelse ($games as $g)
            ...
            @endphp
            <div class="flex items-center gap-2.5 px-3.5 py-3.5 {{ !$loop->last ? 'border-b border-white/[0.05]' : '' }}">
                ...
                <div class="flex-1 min-w-0 flex flex-col gap-2">
                    <div class="flex items-center gap-1.5 min-w-0 {{ $awayWins ? 'text-text-muted' : '' }}">
                        ...
                        <span class="text-[13px] truncate {{ $awayWins ? '' : 'font-semibold' }}">{{ $g['home'] }}</span>
                    </div>
                    <div class="flex items-center gap-1.5 min-w-0 {{ $homeWins ? 'text-text-muted' : '' }}">
                        ...
                        <span class="text-[13px] truncate {{ $homeWins ? '' : 'font-semibold' }}">{{ $g['away'] }}</span>
                    </div>
                </div>
                <div class="flex flex-col items-end flex-none gap-2">
                    <span class="font-mono text-[13.5px] font-bold {{ $awayWins ? 'text-text-muted' : '' }}">{{ $g['home_score'] }}</span>
                    <span class="font-mono text-[13.5px] font-bold {{ $homeWins ? 'text-text-muted' : '' }}">{{ $g['away_score'] }}</span>
                </div>
                @if ($g['result'])
