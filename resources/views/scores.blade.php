                        ...
                        <div data-league-group id="liga-{{ $group['slug'] }}" class="scroll-mt-20">
                            ...
                            <div data-league-body class="bg-surface border border-white/[0.07] rounded-2xl overflow-hidden">
                                @foreach ($group['matches'] as $m)
                                    ...
                                    @endphp
                                    <a href="{{ \App\Support\Nav::match($m['slug']) }}" data-match-row data-match-id="{{ $m['id'] }}" data-status="{{ $m['status'] }}" class="flex items-center gap-3 px-3.5 py-3 border-b border-white/[0.05] last:border-0 hover:bg-white/[0.02]">
                                        ...
                                        {{-- Only while it still helps: once a match is
                                             over, where it was shown is no longer the
                                             question anybody has. --}}
                                        @if (! empty($m['tv']) && $m['status'] === 'scheduled')
                                            <x-tv-chip class="hidden sm:inline-block flex-none">{{ $m['tv'] }}</x-tv-chip>
                                        @endif
