...
<x-layouts.app :sport="$sport" :accent="$accent" :active="$active" :title="'Naslovna — Utakmice.me'">
    ...
    <div class="lg:hidden">
        <div class="px-4 pt-3 flex gap-2">
            <a href="{{ route('home', ['sport' => $sport, 'tab' => 'uzivo']) }}"
            ...
            <a href="{{ route('home', ['sport' => $sport, 'tab' => 'danas']) }}"
            ...
            <a href="{{ route('home', ['sport' => $sport, 'tab' => 'sutra']) }}"
        ...
        <div class="px-4 pt-3.5 flex flex-col gap-2">
            @forelse ($visibleMatches as $m)
                ...
                @endphp
                <a href="{{ route('scores', $sport) }}" class="bg-surface border border-white/[0.07] rounded-2xl p-3.5 flex items-center gap-3.5">
                    ...
                    <div class="flex-1 flex flex-col gap-1.5">
                        <div class="flex justify-between {{ $awayWins ? 'text-text-muted' : '' }}"><span class="text-sm {{ $awayWins ? '' : 'font-semibold' }}">{{ $m['home'] }}</span><span class="font-mono text-sm font-bold">{{ $m['hs'] }}</span></div>
                        <div class="flex justify-between {{ $homeWins ? 'text-text-muted' : '' }}"><span class="text-sm {{ $homeWins ? '' : 'font-semibold' }}">{{ $m['away'] }}</span><span class="font-mono text-sm font-bold">{{ $m['as'] }}</span></div>
                    </div>
                </a>
            @empty
    ...
    <div class="hidden lg:block max-w-[1440px] mx-auto">
        <div class="px-7 py-5 border-b border-white/[0.07] flex flex-col gap-3.5">
            <div class="flex items-center justify-between gap-5">
                ...
                <a href="{{ route('scores', $sport) }}" class="font-mono text-[10.5px] tracking-[0.12em] {{ $accent['text'] }}">SVI REZULTATI &rsaquo;</a>
            </div>
                ...
                <div class="grid grid-cols-4 gap-3">
                    @foreach ($visibleMatches as $m)
                        @php
                            ...
                            $awayWins = $finished && (int) $m['as'] > (int) $m['hs'];
                        @endphp
                        <a href="{{ route('scores', $sport) }}" class="bg-surface border border-white/[0.07] rounded-2xl p-3.5 flex flex-col gap-2.5">
                            ...
                            <div class="flex justify-between items-center {{ $awayWins ? 'text-text-muted' : '' }}"><span class="text-sm {{ $awayWins ? '' : 'font-semibold' }}">{{ $m['home'] }}</span><span class="font-mono text-[15px] font-bold">{{ $m['hs'] }}</span></div>
                            <div class="flex justify-between items-center {{ $homeWins ? 'text-text-muted' : '' }}"><span class="text-sm {{ $homeWins ? '' : 'font-semibold' }}">{{ $m['away'] }}</span><span class="font-mono text-[15px] font-bold">{{ $m['as'] }}</span></div>
                        </a>
                    @endforeach
                </div>
                @if ($hasMoreMatches)
            ...
            @else
                <div class="border border-dashed border-white/[0.12] rounded-2xl p-5.5 flex items-center justify-between gap-5">
                    ...
                    <div class="flex gap-2.5 flex-none">
                        <a href="{{ route('home', ['sport' => $sport, 'tab' => 'danas']) }}" class="h-10 px-4.5 rounded-full flex items-center {{ $accent['bg'] }} text-bg text-[13.5px] font-bold">Vidi današnji program</a>
        ...
        <div class="grid gap-7 px-7 py-6" style="grid-template-columns:1fr 320px">
            <div class="flex flex-col gap-5.5">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('home', $sport) }}" class="font-mono text-[10px] tracking-[0.12em] text-text-muted">ARHIVA &rsaquo;</a>
                </div>
                ...
                <div class="grid gap-4" style="grid-template-columns:1.35fr 1fr">
                    <a href="{{ route('post.show', $hero['slug']) }}" class="bg-surface border border-white/[0.07] rounded-2xl overflow-hidden flex flex-col">
                        <div class="h-75 img-placeholder flex items-end p-3.5" style="height:300px">
                            <span class="font-mono text-[9.5px] tracking-[0.12em] text-text-dim">GLAVNA FOTO 16:9</span>
                        </div>
                        <div class="p-5.5 flex flex-col gap-3">
                            <span class="self-start font-mono text-[9.5px] font-bold tracking-[0.12em] px-2.5 py-1 rounded {{ $accent['tint'] }} {{ $accent['text'] }}">GLAVNA VEST</span>
                    ...
                    <div class="flex flex-col gap-4">
                        @foreach ($secondary as $post)
                            <a href="{{ route('post.show', $post['slug']) }}" class="flex-1 bg-surface border border-white/[0.07] rounded-2xl overflow-hidden flex flex-col">
                                <div class="h-32.5 img-placeholder" style="height:130px"></div>
                ...
                <div class="grid grid-cols-2 gap-x-7 gap-y-4">
                    @foreach ($latest as $post)
                        <a href="{{ route('post.show', $post['slug']) }}" class="flex gap-3.5 items-start">
                            <div class="flex-none w-24 h-18 rounded-[10px] img-placeholder" style="width:96px;height:72px"></div>
                            <div class="flex flex-col gap-1.5">
            ...
            <div class="flex flex-col gap-3.5">
                <div class="bg-surface border border-white/[0.07] rounded-2xl overflow-hidden">
                    <div class="px-3.5 py-3 border-b border-white/[0.07] flex justify-between items-center">
                        ...
                        <a href="{{ route('standings', $sport) }}" class="font-mono text-[9.5px] tracking-[0.1em] text-text-muted">CELA &rsaquo;</a>
                    </div>
